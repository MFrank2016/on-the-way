...
export default function CompletionRateTrendChart() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        console.error('Failed to load completion rate trend:', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  const chartData = useMemo(() => {
    ...
    return trendsData.map((stat) => {
      ...
      return {
        date: dateStr,
        rate: stat.completedTasks > 0 ? 100 : 0,
      }
    })
  ...
  return (
    <div className="bg-white rounded-xl md:rounded-2xl shadow-sm p-4 md:p-6">
      <div className="flex items-center justify-between mb-3 md:mb-4 lg:mb-6">
        <h3 className="text-sm md:text-base lg:text-lg font-semibold text-gray-900">最近完成率趋势</h3>
