...
export default function AchievementScoreChart({ achievementScore }: AchievementScoreChartProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true)
      try {
        const response = await statisticsAPI.getAchievementTrends({ range: timeRange })
        setTrendsData(response.data.data || [])
      } catch (error) {
        console.error('Failed to load achievement trends:', error)
      } finally {
        setLoading(false)
      }
    }
